...
!```bash
if [ -z "$ARGUMENTS" ]; then
  echo "[ERROR] No issue number provided. Usage: /pm:spec-status <issue-number>"
  echo ""
...
gh issue view "$ARGUMENTS" --json number,title,url,body,state,labels \
  --jq '"[OK] Spec issue #\(.number) [\(.state)]: \(.url)\n\(.body)"' \
...
gh api /repos/$REPO/issues/$ARGUMENTS/sub_issues \
  --jq '.[] | "#\(.number) [\(.state | ascii_upcase)] \(.title) \(.html_url)"' \
